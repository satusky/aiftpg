...
async def main():
    ...
    print(f"Loaded {len(gt)} documents from ehr_data.csv")
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=30.0) as client:
        ...
        # Step 3 — submit multiple rounds, all 5 teams concurrently each round
        async def submit(team_name: str, round_num: int) -> dict:
            # Each round gets a unique RNG so results vary between rounds
            seed = hash((team_name, round_num))
            extracted = build_submission(
                gt, team_correctness[team_name], random.Random(seed)
            )
            payload = {
                "team_name": team_name,
        ...
        for round_num in range(1, NUM_ROUNDS + 1):
            ...
            results = await asyncio.gather(
                *(submit(name, round_num) for name in TEAM_NAMES)
            )
